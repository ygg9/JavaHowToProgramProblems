...
public class AirportLocator {
    ...
    // Railroads
    private static Collection<Railroad> railroadList = new ArrayList<Railroad>();

    public static Railroad findClosestRailroad(String zipcode) {
        Railroad result = null;

        Coordinate zipCo = zipcodeMap.get(zipcode);

        double shortestDistance = 10000000000.0;
        for(Railroad railroad: railroadList){
            Coordinate railroadCoord = zipcodeMap.get(Integer.toString(railroad.getZipCode()));
            double distance = zipCo.distanceTo(railroadCoord);
    ...
    private static void loadRailroads() {
        railroadList.add(new Railroad("NW01", "Portland", "OR", 97212));
        railroadList.add(new Railroad("SW02", "Phoenix", "AZ", 85003));
        railroadList.add(new Railroad("NC03", "Rapid City", "SD", 57701));
        railroadList.add(new Railroad("SC04", "San Antonio", "TX", 78202));
        railroadList.add(new Railroad("NE05", "Cleveland", "OH", 44102));
        railroadList.add(new Railroad("SE06", "Jacksonville", "FL", 32202));
    }
